...
import me.meow.Meow;
import me.meow.util.math.MathUtil;
...
public class RotationManager implements Util {
    ...
    public void snapAt(Vec3d directionVec) {
        float[] angle = getRotation(directionVec);
            ...
            if (MathHelper.angleBetween(angle[0], Meow.rotationManager.lastYaw) < 10 && Math.abs(angle[1] - Meow.rotationManager.lastPitch) < 10) {
                return;
            }
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), Meow.rotationManager.rotationYaw, Meow.rotationManager.rotationPitch, mc.player.isOnGround()));
    }
