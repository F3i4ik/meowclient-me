...
import me.meow.Meow;
import me.meow.event.impl.ClientEvent;
...
public class UI
    ...
    public UI() {
        super("UI", "Opens the ClickGui", Module.Category.CLIENT, true, false, false);
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                Meow.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to " + Formatting.DARK_GRAY + Meow.commandManager.getPrefix());
            }
            Meow.colorManager.setColor(this.red.getPlannedValue(), this.green.getPlannedValue(), this.blue.getPlannedValue(), this.hoverAlpha.getPlannedValue());
        }
    }
    ...
    @Override
    public void onLoad() {
        Meow.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        Meow.commandManager.setPrefix(this.prefix.getValue());
    }
