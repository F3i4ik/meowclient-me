package me.meow.features.gui;

import me.meow.Meow;
import me.meow.features.Feature;
import me.meow.features.gui.items.Item;
import me.meow.features.gui.items.buttons.ModuleButton;
...
public class OyVeyGui extends Screen {
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : Meow.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 100, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Meow.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
