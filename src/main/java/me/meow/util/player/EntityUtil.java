package me.meow.util.player;

import me.meow.Meow;
import me.meow.features.modules.misc.MiddleClick;
import me.meow.util.world.BlockPosX;
import me.meow.util.world.BlockUtil2;
import me.meow.util.world.Wrapper;
...
public class EntityUtil implements Wrapper {
    ...
    public static void sendLook(PlayerMoveC2SPacket packet) {
        if (!packet.changesLook() || packet.getYaw(114514) == Meow.rotationManager.lastYaw && packet.getPitch(114514) == Meow.rotationManager.lastPitch) {
            return;
        }
        rotating = true;
        Meow.rotationManager.setRotation(packet.getYaw(0), packet.getPitch(0), true);
